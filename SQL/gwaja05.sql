...
select p.ename, e.job
from (select * from emp where empno = '7788') e join emp p
on e.job = p.job;
...
having avg(sal) =  (select min(avg(sal))
                        from emp
                        group by job)
                        --(select  min(av) from (select avg(sal) as av from emp group by job))
                        ;
...
from emp
where job != 'ANALYST'
and sal not in (select sal from emp where job = 'ANALYST')
;
...
select ename
from emp
where ename NOT IN (select ename from emp where empno in (select distinct mgr from emp))
;
...
select e.ename, deptno, e.job
from emp e natural join dept s
where dname in (select dname from dept where loc = 'DALLAS')
;
