...
--(2) 가격이20,000원이상인도서의이름
    ...
    FROM book
    where price >= 20000;
--(3) 박지성의총구매액(박지성의고객번호는1번으로놓고작성)  xxxxxxxxxxxxxxxxxx
    SELECT saleprice
    FROM orders
    WHERE custid = 1;
    
--(4) 박지성이구매한도서의수(박지성의고객번호는1번으로놓고작성) xxxxxxxxxxxxxxxxxxxxx
    SELECT 
        *
    FROM orders
    where custid = 1;
...
--(1) 마당서점도서의총개수              xxxxxxxxxxxxxxxxxxxxxxxx
    SELECT
        *
    FROM book;
--(2) 마당서점에도서를출고하는출판사의총개수                 xxxxxxxxxxxxxxxxxx
--(3) 모든고객의이름, 주소
    SELECT name, address
